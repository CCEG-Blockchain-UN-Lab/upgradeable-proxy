...
contract('UpgradeCheck', function (accounts) {
    ...
    CheckContract.deployed().then(checkContractInstance => {
      beforeEach(async function() {
        // let checkContractInstance = await CheckContract.new();
        // let proxyInstance = await Proxy.new(checkContractInstance.address);
        ...
        safeProxy = await SafeProxy.new(upgradeCheck_CanUpgrade.address, checkContractInstance.address);
      ...
      it('should be able to deploy SafeProxy with upgradeable contract target', async function () {
        await SafeProxy.new(upgradeCheck_CanUpgrade.address, checkContractInstance.address);
      })
      ...
      it('should not be able to deploy SafeProxy with non-upgradeable contract', async function () {
        try {
          await SafeProxy.new(upgradeCheck_CannotUpgrade.address, checkContractInstance.address);
          throw new Error("This error should not happen")
        } catch (error) {
      ...
      it('should not be able to upgrade to a non-contract', async function () {
        try {
          await SafeProxy.new(accounts[1], checkContractInstance.address);
          throw new Error("This error should not happen")
        } catch (error) {
